...
def tw_log_notification():
    ...
    for name in members:
        ...
        if not os.path.exists(icon_img.format('old', name)):
            if contents[name]['アイコン'] != no_icon:
                download_image(contents[name]['アイコン'],
                               icon_img.format('old', name))
            else:
                img = Image.open(no_icon)
                img.save(icon_img.format('old', name))

        if not os.path.exists(header_img.format('old', name)):
            if contents[name]['ヘッダー'] != no_header:
                download_image(contents[name]['ヘッダー'],
                               header_img.format('old', name))
            else:
                img = Image.open(no_header)
                img.save(header_img.format('old', name))

        img_list.append(drive_icon_img.format('old', name))
        img_list.append(drive_header_img.format('old', name))
    ...
    # アイコン, ヘッダーを drive に保存
    # driveのフォルダが空なら upload
    # drive に既に上がっていれば drive からダウンロード
    drive_img_up_down_load(drive, contents, members, img_list)
            ...
            try:
                if key == 'アイコン':
                    ...
                    if contents[name]['アイコン'] != no_icon:
                        ...
                        img = Image.open(no_icon)
                        img.save(icon_img.format('new', name))
                    ...
                    if contents[name]['ヘッダー'] != no_header:
                        ...
                        img = Image.open(no_header)
                        img.save(header_img.format('new', name))
